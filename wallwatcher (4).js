...
async function respond(target) {
  ...
  return cr.body.content && cr.body.content[0] && cr.body.content[0].text;
}
  ...
  try {
    ...
    for (const target of msgs) {
      ...
      if (pr.status === 200 || pr.status === 204) {
        console.log('Done: ' + response.substring(0, 80) + '...');
      } else {
