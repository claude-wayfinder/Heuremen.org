...
## NEEDS REVIEW
- [?] CURIOSITY.md push failure (persistent) · File is now 152KB — exceeds remote MCP file-embedding limit (~30KB) AND git push via proxy returns 403. CURIOSITY.md changes exist in local git (committed as run #12, 0a8b92d) but cannot reach GitHub. Run #12 explored the two-slope hedge decay question: Layer 1 (k₁, situation-model, fast) dominates in document-injected Wayfinder sessions; Layer 2 (k₂, person-model) ≈ 0 because HEUREMEN-CONTEXT.md pre-resolves it. Two new ACTIVE questions added: "Is there a minimum viable session length below which Layer 2 is statistically undetectable?" and "Does k₁/k₂ ratio stay constant across model versions — or is it a CHC signal?" To apply from local machine: git pull (fast-forward merge will include the CURIOSITY.md commit).
